...
def kfold_lightgbm(params,df, predictors,target,num_folds, stratified = True,
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['target'])):
        ...
        print('Fold %2d raw AUC : %.6f' % (n_fold + 1, 
                        roc_auc_score(valid_y, oof_raw_preds[valid_idx])))
        print('Fold %2d denoised raw AUC : %.6f' % (n_fold + 1, 
                        roc_auc_score(valid_y, oof_denoise_preds[valid_idx])))
        print('Fold %2d raw + augmented AUC : %.6f' % (n_fold + 1, 
                        roc_auc_score(valid_y, oof_dn_aug_preds[valid_idx])))
        print('Fold %2d raw + denoised augmented AUC : %.6f' % (n_fold + 1, 
                        roc_auc_score(valid_y, oof_raw_dn_aug_preds[valid_idx])))
        print('Fold %2d denoised aug AUC : %.6f' % (n_fold + 1, 
                        roc_auc_score(valid_y, oof_raw_dn_aug_preds[valid_idx])))
    ...
    score = roc_auc_score(train_df['target'], oof_denoise_preds)
    train_df['predict'] = oof_denoise_preds
    train_df[['ID_code', 'predict']].to_csv('../res/'+'denoise_val_{}.csv'.format(score), index= False)
    test_df[target] = sub_denoise_preds
    test_df[['ID_code', target]].to_csv('../res/'+'denoise_sub_{}.csv'.format(score), index= False)

       
    # Write submission file and plot feature importance
    score = roc_auc_score(train_df['target'], oof_aug_preds)
    train_df['predict'] = oof_aug_preds
    train_df[['ID_code', 'predict']].to_csv('../res/'+'aug_val_{}.csv'.format(score), index= False)
    test_df[target] = sub_aug_preds
    test_df[['ID_code', target]].to_csv('../res/'+'aug_sub_{}.csv'.format(score), index= False)
    ...
    print('Full raw AUC score %.6f' % roc_auc_score(train_df['target'], 
                    oof_raw_preds))
    print('Full denoised raw AUC score %.6f' % 
            roc_auc_score(train_df['target'], oof_denoise_preds))
    
    print('Full raw + augmented AUC score %.6f' % 
            roc_auc_score(train_df['target'], oof_aug_preds)) 

    print('Full denoised augmented AUC score %.6f' % roc_auc_score(train_df['target'], 
                    oof_dn_aug_preds))
    print('Full Raw + denoised augmented AUC score %.6f' %
                     roc_auc_score(train_df['target'], oof_raw_dn_aug_preds))
...
def run():
    ...
    #             'var_68_no_noise']
    no_use = []
# [9117]  train's auc: 0.973287   valid's auc: 0.923222 first fold without removing above features                
    target = 'target'    
